...
class TestFileDecompressRawMapper(TestCase):
    ...
    def test_should_check_filename(self) -> None:
        ...
        response = self.mapper.decompress_raw(content=content)
        ...
        self.assertEqual(call_args_list[1][1], {"message": f"Decompress raw filename [{filename}]."})

    def test_should_check_content(self) -> None:
        content = self.faker.zip(uncompressed_size=64, num_files=1, min_file_size=32, compression="deflate")
    ...
    def test_should_check_logger_info(self) -> None:
        ...
        self.mapper.decompress_raw(content=content)
        ...
        self.assertEqual(call_args_list[0][1], {"message": f"Try to decompress raw [{len(content)}] bytes."})
    ...
    def test_should_check_throws_exception_when_try_to_decompress_xz_content(self) -> None:
        ...
        self.assertEqual(context.exception.args[0], "Error to decompress raw content.")
        ...
        self.assertEqual(call_args_list[0][1]["message"], "Error to decompress raw content.")
